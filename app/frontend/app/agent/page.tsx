...
export default function AgentPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-8 pb-16">
    ...
    <section aria-labelledby="recent-work-title"><div className="mb-3 flex items-center justify-between"><h2 id="recent-work-title" className="text-[10px] font-black uppercase tracking-[.18em] text-muted">Recent Work</h2><Button asChild variant="ghost" size="sm"><Link href="/history">Full history <ArrowRight className="size-3.5"/></Link></Button></div>{workspace?.recent_work.length ? <div className="overflow-hidden rounded-2xl border border-line bg-white">{workspace.recent_work.slice(0, 6).map((entry, index) => <div key={entry.id} className={`flex items-start gap-3 p-4 ${index ? "border-t border-line" : ""}`}><Clock3 className="mt-0.5 size-4 shrink-0 text-[#9b7521]"/><div className="min-w-0"><p className="text-sm font-semibold leading-5">{entry.message}</p><p className="mt-1 text-[10px] capitalize text-muted">{new Date(entry.timestamp).toLocaleString()} · {entry.source_channel}</p></div></div>)}</div> : <Card><CardContent className="py-10 text-center"><p className="text-sm font-semibold">No completed work yet</p><p className="mt-1 text-xs text-muted">Finished assignments will appear here.</p></CardContent></Card>}</section>
  </div>;
}
